...
(defun bingo (nums boards marks)
  ...
  (when (= (length boards) 0) (error "Broke"))
  (if (and (= (length boards) 1) (winner (first marks)))
      (score (first boards) (first marks) (first nums))
      (bingo (rest nums)
             (loop for n from 0 below (length boards) append (if (winner (elt marks n)) () (list (elt boards n))))
             (remove-if #'winner marks))))
