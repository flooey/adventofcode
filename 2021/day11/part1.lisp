...
(defun clear-flashed (a)
  (apply #'+ (loop for i from 0 below (array-total-size a)
                   collect (if (> (row-major-aref a i) 9)
                               (+ 1 (setf (row-major-aref a i) 0))
                               0))))
...
(print (apply #'+ (loop for i below 100 collect (run-loop *input*))))
