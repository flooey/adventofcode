...
(defun has-double (used)
  (not (equal (remove-duplicates used :test #'equal) used)))

(defun can-visit (used d)
  (cond ((not (find d used :test #'equal)) t)
        ((has-double used) nil)
        (t t)))
  ...
  (let ((newused (if (lower-case-p (elt pos 0))
    ...
    (if (equal pos "end")
        ...
        (loop for d in (paths-from paths pos)
              sum (if (not (can-visit newused d))
                      0
